...
export async function GET() {
  ...
  const result: Record<string, unknown> = { testMode: true, supabaseUrl: Boolean(process.env.NEXT_PUBLIC_SUPABASE_URL), supabaseServiceKey: Boolean(process.env.SUPABASE_SERVICE_ROLE_KEY), openaiKey: Boolean(process.env.OPENAI_API_KEY), model: process.env.OPENAI_MODEL?.trim() || "gpt-5.6-luna", steps: {} };
  ...
  try {
    ...
    const now = Date.now();
    const identity = new ExternalTrustedAuthAdapter(issuer).verify({ subject: userId, role: "admin", issuer, issuedAt: now, expiresAt: now + 300000, nonce: randomUUID(), scopes: ["luna:*"] });
    ...
    const { data: task, error: taskError } = await supabase.from("tasks").insert({ user_id: userId, title: "LUNA-DIAGNOSTIC-TASK", status: "todo", priority: 3, description: "diagnostic" }).select("id").single();
