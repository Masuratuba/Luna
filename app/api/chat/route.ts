...
export async function POST(request: Request) {
  try {
    ...
    if (conversationId) {
      ...
      const { data: conversation, error } = await supabase
        .from("conversations")
        .insert({ user_id: user.id, title: message.slice(0, 80) || "Neue Unterhaltung" })
        .select("id")
        .single();
      if (error) throw error;
    ...
    const guardEvent = createEvent("guard.checked", user.id, {
      ...
      role,
      trustedAdmin: Boolean(trustedAdmin),
      risk: guard.risk,
    });
    ...
    if (core.decision === "CREATE_TASK" || core.decision === "SAVE_MEMORY") {
      ...
      if (core.decision === "SAVE_MEMORY" && !explicitMemory) {
        ...
        const { error: actionInsertError } = await supabase
          .from("luna_actions")
          .insert({ id: action.id, user_id: user.id, type: action.type, status: action.status, input: action.input });
        if (actionInsertError) throw actionInsertError;
        await persistEvent(
          supabase,
          user.id,
          createEvent("action.created", user.id, { actionId: action.id, type: action.type, agent: core.agent }),
          "allowed",
          guard.risk,
        );
            ...
            executeDomainAction: async (domainAction) => {
              if (domainAction.type === "task") {
                ...
                const { data, error } = await supabase
                  .from("tasks")
                  .insert({ user_id: user.id, title, description: message, status: "todo", priority: 3, due_at: null })
                  .select()
                  .single();
                if (error) throw error;
              ...
              if (domainAction.type === "memory") {
                ...
                const { data, error } = await supabase
                  .from("memories")
                  .insert({ user_id: user.id, type: "instruction", content, importance: 1, metadata: { source: "explicit_user_instruction", saved_via: "chat_action_engine" } })
                  .select("id")
                  .single();
                if (error) throw error;
        ...
        if (gateway.execution) {
          ...
          const { error } = await supabase
            .from("luna_actions")
            .update({ status: gateway.execution.action.status, output: actionOutput })
            .eq("id", action.id)
            .eq("user_id", user.id);
          if (error) throw error;
        } else {
          ...
          const { error } = await supabase
            .from("luna_actions")
            .update({ status: "failed", output: { error: actionError } })
            .eq("id", action.id)
            .eq("user_id", user.id);
          if (error) throw error;
        }

        const completionType = actionStatus === "completed" ? "action.completed" : "action.failed";
        const actionEvent = createEvent(completionType, user.id, {
          actionId: action.id,
          type: action.type,
          status: actionStatus,
          output: actionOutput,
    ...
    return NextResponse.json({
      ...
      memorySaved: actionOutput.type === "memory" && actionStatus === "completed",
      searchPerformed,
      reply,
    });
  } catch (error: unknown) {
