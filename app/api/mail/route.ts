...
export async function POST(request: Request) {
  try {
    ...
    const provider = createProviderRegistry().mail();
    ...
    if (operation === "search") {
      ...
      const result = await executeThroughGuardian({
        agent: "research", capability: "mail.read", mode: "read", action,
        context: { authenticated: true, userId: user.id, role, trustedAdmin, identity, budget, handler: async () => ({ messages: await provider.search({ query, limit: body.limit, folder: body.folder }) }) },
      });
    ...
    if (operation === "read") {
      ...
      const result = await executeThroughGuardian({
        agent: "research", capability: "mail.read", mode: "read", action,
        context: { authenticated: true, userId: user.id, role, trustedAdmin, identity, budget, handler: async () => ({ message: await provider.read({ id }) }) },
      });
      if (!result.ok) return NextResponse.json({ ok: false, error: result.error ?? result.guard.reason }, { status: 403 });
    ...
    if (operation === "send") {
      ...
      if (body.approved !== true || typeof body.confirmationToken !== "string" || !body.confirmationToken.trim()) {
        return NextResponse.json({ ok: false, approvalRequired: true, error: "mail sending requires explicit approval and confirmationToken" }, { status: 403 });
      }
      const action = createAction("tool", { tool: "mail.send", operation, to, cc, subject, body: messageBody });
      const result = await executeThroughGuardian({
        agent: "action", capability: "mail.send", mode: "execute", action,
        context: { authenticated: true, userId: user.id, role, trustedAdmin, identity, approved: true, confirmationToken: body.confirmationToken.trim(), budget, handler: async () => ({ result: await provider.send({ to, cc, subject, body: messageBody }) }) },
      });
      if (!result.ok) return NextResponse.json({ ok: false, error: result.error ?? result.guard.reason }, { status: 403 });
