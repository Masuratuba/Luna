...
import { LUNA_SYSTEM_PROMPT } from "./prompt";
import { runLunaCore, createAction, createEvent, createAuditEntry } from "./core";
import { assessIntelligence } from "./intelligence-core";
import { evaluateGuard } from "./guard";
import { extractExplicitMemory, selectRelevantMemories } from "./memory";
...
import { executeLunaCommand, parseLunaCommand } from "./command-capabilities";
import { getLunaAgent } from "./agents";
...
async function handleChatPost(request: Request, dependencies: ChatDependencies = createDefaultChatDependencies()) {
  try {
    ...
    if (directCommand) {
      if (directCommand.kind === "forget" || directCommand.kind === "update") {
        const action = createAction("memory", { message, agent: "memory", command: directCommand.kind });
        ...
        const guardianResult = await executeThroughGuardian({
          ...
          context: {
            ...
            identity,
            budget,
            handler: async () => directCommand.kind === "forget"
              ? forgetMemory(supabase, user.id, message)
              : updateMemory(supabase, user.id, message),
          },
        });
    ...
    const [{ data: recentMessages, error: messagesError }, { data: memories, error: memoryError }] = await Promise.all([
      ...
      supabase.from("memories").select("type, content, importance").eq("user_id", user.id).order("importance", { ascending: false }).order("updated_at", { ascending: false }).limit(50),
    ]);
    ...
    const memoryExecutionContext = core.decision === "SAVE_MEMORY"
      ? memorySaved
        ? "MEMORY OPERATION RESULT: DURABLY SAVED. Tell the user clearly that the memory was saved permanently. Do NOT say that durable storage cannot be confirmed. Do NOT say this is only for the current chat."
