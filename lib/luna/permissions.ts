...
const DEFAULT_PERMISSIONS: ToolPermission[] = [
  ...
  { tool: "calendar.read", level: "read", requiresConfirmation: false },
  ...
  { tool: "calendar.write", level: "destructive", requiresConfirmation: true },
  { tool: "data.delete", level: "destructive", requiresConfirmation: true },
];
export function getToolPermission(tool: string): ToolPermission { return DEFAULT_PERMISSIONS.find((permission) => permission.tool === tool) ?? { tool, level: "destructive", requiresConfirmation: true }; }
export function canPerformAction(action: LunaAction, authenticated: boolean, tool?: string) { if (!authenticated) return false; if (!tool) return true; return !getToolPermission(tool).requiresConfirmation && action !== "delete"; }
