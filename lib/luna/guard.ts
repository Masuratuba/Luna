...
/** Admin is trusted only when an external authentication layer has already established adminAuthenticated=true. */
export function checkGuard(request: GuardRequest): GuardResult {
  const risk = classifyRisk(request.action);
  const trustedAdmin = request.role === "admin" && request.adminAuthenticated === true;
...
export type GuardInput = { userId: string; message: string; decision: LunaDecision; toolName?: string; role?: GuardRole; adminAuthenticated?: boolean };
export function evaluateGuard(input: GuardInput): GuardResult {
  const trustedAdmin = input.role === "admin" && input.adminAuthenticated === true;
